<template>
  <div class="overflow-hidden">
    <div v-if="!selectedMovie" @click="toggleDropdown" class="relative transition-all w-full rounded h-20 flex items-center text-sm px-2 cursor-pointer bg-black">{{ $t('message.choose_movie')}}</div>
    <div v-if="selectedMovie" class="group cursor-pointer relative rounded overflow-hidden w-full" @click="toggleDropdown">
      ...
      <h2 class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:text-xl text-sm select-none">{{$t('message.selected')}}: {{selectedMovie.title[languageStore.getLanguage]}}</h2>
    </div>
    <div class="transition-all overflow-y-auto" :class="dropdownOpen ? 'max-h-80 ' : 'max-h-0'">
      <div v-for="movie in props.movies" :key="movie.id" @click="setMovie(movie)" class="group cursor-pointer relative">
        ...
        <h2 class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:text-3xl text-md select-none">{{movie.title[languageStore.getLanguage]}}</h2>
      </div>
    </div>
  </div>
</template>
